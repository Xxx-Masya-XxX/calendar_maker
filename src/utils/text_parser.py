...
def parse_spec_days_text(text: str, default_bg: str = "", default_color: List[int] = None) -> List[Dict]:
    ...
    # Dictionary to collect entries by date (to merge multiple names per date)
    ...
    for line in lines:
        ...
        if date_match and current_month:
            ...
            # Collect names by date
            if full_date not in entries_by_date:
                entries_by_date[full_date] = []
            entries_by_date[full_date].append(name)

    # Build the result list
    result = []
    for date, names in entries_by_date.items():
        entry = {
            "date": date,
            "name": "День рождения",
            "desc": "\n".join(names),
